...
The goose commands are a consistent `daryl-` family (bare `daryl` is the
primary non-goose Daryl):

- `daryl-goose [dir]` — goose Daryl on your default provider (Claude).
- `daryl-gpt [dir]` — goose Daryl on GitHub Copilot's flagship GPT (`gpt-5.6-sol`).
  Override the tier: `GOOSE_MODEL=gpt-5.6-terra daryl-gpt` (sol = flagship,
  terra = balanced, luna = fast/cheap).
- `daryl-gemini [dir]` — goose Daryl on Gemini via Copilot (`gemini-3.1-pro-preview`).
