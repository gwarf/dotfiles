...
# Show status against remote, displaying commit changes counts
_remote_changes() {
  local -a remote
  ...
  if [[ -n ${remote} ]] ; then
    ...
    if [ $ahead -eq 0 ] && [ $behind -gt 0 ]; then
      echo " %{$fg_bold[magenta]%}↓(${behind})%{$reset_color%}"
    elif [ $ahead -gt 0 ] && [ $behind -eq 0 ]; then
      echo " %{$fg_bold[magenta]%}↑(${ahead})%{$reset_color%}"
    elif [ $ahead -gt 0 ] && [ $behind -gt 0 ]; then
      echo " %{$fg_bold[magenta]%}↕: ↓(${behind}) ↑(${ahead})%{$reset_color%}"
    fi
  fi
}
...
PROMPT='%{%f%k%b%}
%{%K{${bkg}}%B%F{green}%}%n%{%B%F{blue}%}@%{%B%F{cyan}%}%m%{%B%F{green}%} %{%b%F{yellow}%K{${bkg}}%}%3c%{%B%F{green}%}$(git_prompt_info)$(_remote_changes)$(_untracked)%E%{%f%k%b%}
